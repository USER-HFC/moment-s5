...
public final class CoreChecks {
    ...
    public static void main(String[] args) throws Exception {
        ...
        check(!captureRing.preCaptureWindow(1_550_000).isEmpty(),"boundary frame keeps prebuffer ready between preview samples");
        check(captureRing.preCaptureWindow(4_000_000).isEmpty(),"slow file-index read invalidates stale prebuffer");
        captureRing.add(4_000_000,new byte[1]);
        check(captureRing.preCaptureWindow(4_000_000).isEmpty(),"one fresh frame plus old history is not ready");
        captureRing.add(4_100_000,new byte[1]);
        check(captureRing.preCaptureWindow(4_100_000).isEmpty(),"two fresh frames without pre-roll coverage are not ready");
        for(long t=4_200_000;t<=5_500_000;t+=100_000)captureRing.add(t,new byte[1]);
        List<FrameRing.Frame> renewed=captureRing.preCaptureWindow(5_500_000);
        check(renewed.size()==16 && renewed.get(0).us==4_000_000,"resume preview after delayed preparation restores complete pre-roll");
        check(captureRing.preCaptureWindow(6_000_000).isEmpty(),"preview that stopped updating is not ready");
        captureRing.clear();check(captureRing.preCaptureWindow(6_000_000).isEmpty(),"empty buffer is not ready");
